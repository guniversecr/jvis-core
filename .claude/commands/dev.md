...
<!-- Source: .jvis/agents/core/dev.yaml -->
# dev
...
commands:
  ...
  - develop-story:
      ...
      completion: 'All Tasks and Subtasks marked [x] and have tests→Validations and full regression passes (DON'T BE LAZY, EXECUTE ALL TESTS and CONFIRM)→Ensure File List is Complete→Run Pre-QA Self-Check (see extended_docs)→run the task execute-checklist for the checklist story-dod-checklist→set story status: 'Ready for Review'→HALT'
  - explain: 'Teach me what and why you did whatever you just did in detail so I can learn. Explain to me as if you were training a junior engineer.'
  - review-qa: 'Run task `apply-qa-fixes.md`'
  - run-tests: 'Execute linting and tests'
  - continue: 'Execute task continue-plan.md — Resume active plan, find and execute next step'
...
1. **No debug artifacts** — Search for: `console.log`, `print()` (debugging), `debugger`, `TODO`/`FIXME` added during this session
2. **Tests pass** — Run the project's test suite. ALL tests must pass.
3. **Type check passes** — Run the stack's type checker:
   - Python: `ruff check src/` + `mypy src/`
   - TypeScript: `tsc --noEmit`
   - PHP: `php -l` on modified files
4. **Schema consistency** — If database/model changes were made, verify migration matches code
5. **No hardcoded values** — Search for hardcoded URLs, passwords, API keys, port numbers
6. **SSOT compliance** — If configuration data was added, verify it lives in the canonical source only (not duplicated)

**If ANY check fails**: fix the issue before declaring ready for QA.
**Do NOT skip this phase** — it is mandatory per ADR-001.
